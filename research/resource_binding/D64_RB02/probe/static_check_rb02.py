...
def main()->int:
 ...
 det=block(t,'binding_detach','binding_detach_done'); clear_pos=det.find('movb $0,binding_resource_plus1(%di)'); dec_pos=det.find('decw resource_live_count(%si)'); zero_after=det.rfind('cmpw $0,resource_live_count(%si)'); c['detach_lifetime_order']=bool(clear_pos>=0 and dec_pos>clear_pos and zero_after>dec_pos and 'resource_generation' not in det)
 ...
 c['host_no_guest_mutation_or_trace_synthesis']=bool(all(x not in l for x in ['binding_resource_plus1(','resource_live_count(','act_identity(','resource_identity(','debug.write_text','debug.write_bytes']) and all(x not in e for x in ['binding_resource_plus1(','resource_live_count(','act_identity(','resource_identity(','debug.write_text','debug.write_bytes']))
