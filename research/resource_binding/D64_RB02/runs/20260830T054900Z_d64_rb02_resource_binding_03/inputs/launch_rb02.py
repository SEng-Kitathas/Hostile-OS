...
STAGE2_EXTENT=8192
QEMU_TIMEOUT=12
PREREG_COMMIT='0d0305ebde56d7a9eb70a3db84b00fcf96b33c17'
...
def main():
 ...
 originals={'preregistration':src.parent/'D64_RB02_PREREGISTRATION.md','target_profile':repo/'research/plans/HOSTILE_OS_TARGET_WORKLOAD_PROFILE_D64_2026-08-30.md','resource_plan':repo/'research/plans/D64_RESOURCE_BINDING_SCALE_PLAN_2026-08-30.md','width_correction':repo/'research/plans/D64_RESOURCE_BINDING_LIVE_COUNT_WIDTH_CORRECTION_2026-08-30.md','loader_qualification_result':qual/'QUALIFICATION_RESULT.md','stage1_s':qual/'stage1.S','stage1_ld':qual/'stage1.ld','stage2_s':src/'stage2.S','stage2_ld':src/'stage2.ld','launcher':Path(__file__).resolve(),'evaluator':src/'evaluate_rb02.py','static_checker':src/'static_check_rb02.py'}
 snap={'preregistration':'preregistration.md','target_profile':'target_profile.md','resource_plan':'resource_plan.md','width_correction':'width_correction.md','loader_qualification_result':'loader_qualification_result.md','stage1_s':'stage1.S','stage1_ld':'stage1.ld','stage2_s':'stage2.S','stage2_ld':'stage2.ld','launcher':'launch_rb02.py','evaluator':'evaluate_rb02.py','static_checker':'static_check_rb02.py'}
 head=git_head(repo); items=[]; original_hash={}
 for k,p in originals.items():
  d=inp/snap[k]; shutil.copyfile(p,d); original_hash[k]=sha(p); items.append({'key':k,'source_project_relative':p.relative_to(repo).as_posix(),'snapshot_path':d.relative_to(run).as_posix(),'bytes':d.stat().st_size,'sha256':sha(d)})
 manifest={'run_id':rid,'snapshot_utc':now(),'controlling_git_head':head,'controlling_preregistration_commit':PREREG_COMMIT,'declared_working_directory':str(repo),'launcher_path':str(originals['launcher']),'launcher_sha256':original_hash['launcher'],'tools':{k:str(v) for k,v in {'clang':CLANG,'lld':LLD,'objcopy':OBJCOPY,'size':SIZE,'nm':NM,'qemu':QEMU,'python':PYTHON}.items()},'inputs':items}
 mp=run/'inputs_manifest.json'; mp.write_text(json.dumps(manifest,indent=2)+'\n',encoding='utf-8'); msha=sha(mp)
 s1s=inp/'stage1.S'; s1ld=inp/'stage1.ld'; s2s=inp/'stage2.S'; s2ld=inp/'stage2.ld'; evaluator=inp/'evaluate_rb02.py'; checker=inp/'static_check_rb02.py'; launcher_snap=inp/'launch_rb02.py'
 ...
 changed=[k for k,p in originals.items() if sha(p)!=original_hash[k]]
 if changed:
 ...
 source={i['key']:i['sha256'] for i in items}; observed_max=None
 if dbg.exists():
  for line in dbg.read_text(encoding='ascii',errors='replace').splitlines():
   if line.startswith('SHARE_COUNT='):
    try: observed_max=int(line.split('=',1)[1],16)
    except ValueError: pass
 pre={'run_id':rid,'run_class':'D64_RB02_RESOURCE_BINDING_SCALE','scientific_status':qstatus,'authority_ceiling':'bounded 64x20 binding / 64-resource relation scale only','input_manifest_sha256':msha,'controlling_git_head':head,'controlling_preregistration_commit':PREREG_COMMIT,'qemu':{'pid':pid,'argv':qargv,'started_utc':started,'ended_utc':ended,'wall_ms':wall,'status':qstatus,'exit_code':qexit,'timeout_seconds':QEMU_TIMEOUT},'source_sha256':source,'tools':{k:{'path':str(v),'sha256':sha(v)} for k,v in {'clang':CLANG,'lld':LLD,'objcopy':OBJCOPY,'size':SIZE,'nm':NM,'qemu':QEMU,'python':PYTHON}.items()},'artifacts':{'stage1_bin':{'bytes':len(b1),'sha256':sha(s1bin),'boot_signature':'55aa'},'stage2_raw':{'bytes':len(b2),'sha256':sha(s2raw)},'stage2_padded':{'bytes':len(s2pad.read_bytes()),'sha256':sha(s2pad)},'disk':{'bytes':len(img),'sha256':sha(disk)},'debugcon':{'sha256':sha(dbg) if dbg.exists() else None},'evaluation':{'sha256':sha(evaluation) if evaluation.exists() else None,'exit':eval_exit},'size':{'sha256':sha(size_so),'exit':size_exit},'nm':{'sha256':sha(nm_so),'exit':nm_exit}},'pareto':{'stage2_raw_bytes':len(b2),'stage2_extent_bytes':STAGE2_EXTENT,'runtime_state_bytes':runtime,'activity_capacity':64,'bindings_per_activity':20,'binding_cell_count':1280,'resource_capacity':64,'resource_live_count_bits':16,'max_observed_resource_live_count':observed_max,'binding_generation_bits':8,'resource_generation_bits':8,'max_binding_row_scan':20,'max_resource_scan':64}}
 ...
 print('RUN_DIR='+str(run)); print(f'QEMU_PID={pid} STATUS={qstatus} EXIT={qexit} WALL_MS={wall:.3f}'); print('TRACE='+repr(dbg.read_text(encoding='ascii').splitlines() if dbg.exists() else [])); print(f'EVALUATOR_EXIT={eval_exit} STATIC_EXIT={static_exit} STAGE2_RAW_BYTES={len(b2)} RUNTIME_STATE_BYTES={runtime} MAX_SHARE={observed_max}'); print('INPUT_MANIFEST_SHA256='+msha); print('RECEIPT_SHA256='+sha(rp))
