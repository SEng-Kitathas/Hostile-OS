...
def main() -> int:
    ...
    clear_pos = detach.find("movb $0,binding_resource_plus1(%di)")
    dec_pos = detach.find("decw resource_live_count(%si)", clear_pos + 1)
    zero_pos = detach.find("cmpw $0,resource_live_count(%si)", dec_pos + 1)
    id_clear_pos = detach.find("movb $0,resource_identity(%bx)", zero_pos + 1)
    value_clear_pos = detach.find("movb $0,resource_value(%bx)", id_clear_pos + 1)
    c["detach_clear_ref_before_decrement_and_reclaim_on_zero"] = bool(
        clear_pos >= 0 and clear_pos < dec_pos < zero_pos < id_clear_pos < value_clear_pos
    )
    ...
    pre_bind = boot2.find("movb %al,o_old_bind_pre")
    pre_res = boot2.find("movb %al,o_old_res_pre", pre_bind + 1)
    acq = boot2.find("call activity_acquire", pre_res + 1)
    rebind = boot2.find("call bind_new_resource", acq + 1)
    post_bind = boot2.find("movb %al,o_old_bind_post", rebind + 1)
    post_res = boot2.find("movb %al,o_old_res_post", post_bind + 1)
    c["old_handles_checked_pre_and_post_rebind"] = bool(
        pre_bind >= 0 and pre_bind < pre_res < acq < rebind < post_bind < post_res
        and boot2.rfind("call binding_read", 0, pre_bind) >= 0
        and boot2.rfind("call resource_read", 0, pre_res) >= 0
        and boot2.rfind("call binding_read", rebind, post_bind) >= rebind
        and boot2.rfind("call resource_read", post_bind, post_res) >= post_bind
    )
    ...
    c["launcher_two_distinct_qemu_processes_strict_order"] = bool(
        l.count("run_qemu(disk,") == 2
        and "proc = subprocess.Popen(argv" in l
        and "distinct_pids = boot1_pid != boot2_pid" in l
        and "strict_order = boot2_started_monotonic >= boot1_ended_monotonic" in l
    )
