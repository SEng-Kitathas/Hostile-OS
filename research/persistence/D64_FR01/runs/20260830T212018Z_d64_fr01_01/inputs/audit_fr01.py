...
def main()->int:
 ...
 no_value=True; success_handles=True; all_traces_have_select=True
 for x in c['fixtures']:
  lines=(r/x['relative_dir']/'debugcon.txt').read_text(encoding='ascii',errors='replace').splitlines(); sel=next((v.split('=',1)[1] for v in lines if v.startswith('SELECT=')),None)
  if sel is None:
   all_traces_have_select=False; no_value=False; success_handles=False; continue
 ...
 checks={
  ...
  'all_traces_have_select':bool(all_traces_have_select),
