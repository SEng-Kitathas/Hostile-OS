...
def main()->int:
 ...
 source_map={
  ...
  'prereg':'research/persistence/D64_FR01/D64_FR01_PREREGISTRATION.md','amendment_a':'research/persistence/D64_FR01/D64_FR01_PREREGISTRATION_AMENDMENT_A.md','mechanism_selection':'research/persistence/D64_FR01/D64_FR01_INTEGRITY_MECHANISM_SELECTION.md','parent_plan':'research/plans/D64_FAULTED_RESTART_DURABLE_RECORD_PLAN_2026-08-30.md','pr01_prereg':'research/persistence/D64_PR01/D64_PR01_PREREGISTRATION.md','pr01_result':'research/persistence/D64_PR01/D64_PR01_RESULT_2026-08-30.md'}
 records=[]
 ...
 all_pass=ev==0 and st==0 and au==0 and all(x['qemu']['status']=='COMPLETED' and x['qemu']['exit_code']==33 for x in results) and campaign['original_inputs_unchanged']; summary={'run_id':run_id,'git_head':git_head,'fixture_count':len(results),'evaluator_exit':ev,'static_exit':st,'audit_exit':au,'all_pass':all_pass,'stage2_bytes':len(s2),'campaign_receipt_sha256':sha(run/'campaign_receipt.json'),'evaluation_sha256':sha(run/'evaluation.json'),'static_sha256':sha(run/'static_closure.json'),'audit_sha256':sha(run/'independent_audit.json')}; (run/'result_summary.json').write_text(json.dumps(summary,indent=2)+'\n',encoding='utf-8',newline='\n'); print(json.dumps(summary,indent=2)); return 0 if all_pass else 1
