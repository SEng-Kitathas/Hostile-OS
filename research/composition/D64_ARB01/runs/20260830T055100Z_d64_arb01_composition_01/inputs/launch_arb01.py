...
STAGE2_EXTENT=8192
QEMU_TIMEOUT=12
PREREG_COMMIT='c7170dc018e463e6220bb7ce39e9018950e65754'
...
def main():
 ...
 originals={
  'preregistration':src.parent/'D64_ARB01_PREREGISTRATION.md',
  'composition_plan':repo/'research/plans/D64_ACTIVITY_REKEY_BINDING_COMPOSITION_PLAN_2026-08-30.md',
  'rk01_result':repo/'research/rekey/D64_RK01/D64_RK01_RESULT.md',
  'rk01_adoption':repo/'research/architecture/D64_RK01_REKEY_ADOPTION_REVIEW_2026-08-30.md',
  'rb02_result':repo/'research/resource_binding/D64_RB02/D64_RB02_RESULT.md',
  'rb02_correction':repo/'research/resource_binding/D64_RB02/D64_RB02_PROVENANCE_CORRECTION_2026-08-30.md',
  ...
  'evaluator':src/'evaluate_arb01.py','static_checker':src/'static_check_arb01.py'}
 snap={k:(k+'.md' if k in ['preregistration','composition_plan','rk01_result','rk01_adoption','rb02_result','rb02_correction','loader_qualification_result'] else {'stage1_s':'stage1.S','stage1_ld':'stage1.ld','stage2_s':'stage2.S','stage2_ld':'stage2.ld','launcher':'launch_arb01.py','evaluator':'evaluate_arb01.py','static_checker':'static_check_arb01.py'}[k]) for k in originals}
 ...
 s1s=inp/'stage1.S'; s1ld=inp/'stage1.ld'; s2s=inp/'stage2.S'; s2ld=inp/'stage2.ld'; evaluator=inp/'evaluate_arb01.py'; checker=inp/'static_check_arb01.py'; launcher_snap=inp/'launch_arb01.py'
 ...
 source={i['key']:i['sha256'] for i in items}; obs={}
 if dbg.exists():
  for line in dbg.read_text(encoding='ascii',errors='replace').splitlines():
   if '=' in line: k,v=line.split('=',1); obs[k]=v
 pre={'run_id':rid,'run_class':'D64_ARB01_ACTIVITY_REKEY_BINDING_COMPOSITION','scientific_status':qstatus,'authority_ceiling':'bounded D64 activity-rekey plus binding relation composition only','input_manifest_sha256':msha,'controlling_git_head':head,'controlling_preregistration_commit':PREREG_COMMIT,'qemu':{'pid':pid,'argv':qargv,'started_utc':started,'ended_utc':ended,'wall_ms':wall,'status':qstatus,'exit_code':qexit,'timeout_seconds':QEMU_TIMEOUT},'source_sha256':source,'tools':{k:{'path':str(v),'sha256':sha(v)} for k,v in {'clang':CLANG,'lld':LLD,'objcopy':OBJCOPY,'size':SIZE,'nm':NM,'qemu':QEMU,'python':PYTHON}.items()},'artifacts':{'stage1_bin':{'bytes':len(b1),'sha256':sha(s1bin),'boot_signature':'55aa'},'stage2_raw':{'bytes':len(b2),'sha256':sha(s2raw)},'stage2_padded':{'bytes':len(s2pad.read_bytes()),'sha256':sha(s2pad)},'disk':{'bytes':len(img),'sha256':sha(disk)},'debugcon':{'sha256':sha(dbg) if dbg.exists() else None},'evaluation':{'sha256':sha(evaluation) if evaluation.exists() else None,'exit':eval_exit},'size':{'sha256':sha(size_so),'exit':size_exit},'nm':{'sha256':sha(nm_so),'exit':nm_exit}},'pareto':{'stage2_raw_bytes':len(b2),'stage2_extent_bytes':STAGE2_EXTENT,'runtime_state_bytes':runtime,'activity_capacity':64,'bindings_per_activity':20,'binding_cell_count':1280,'resource_capacity':64,'activity_identity_scan_bound':64,'checked_release_row_scan_bound':20,'rekey_binding_scan_bound':1280,'rekey_resource_scan_bound':64,'activity_reset_bound':64,'binding_reset_bound':1280,'observed_new_activity_epoch':obs.get('NEW_EPOCH'),'observed_resource_epoch':obs.get('RES_EPOCH'),'observed_resource_generation_after_rekey':obs.get('RES_GEN')}}
 ...
 print('RUN_DIR='+str(run)); print(f'QEMU_PID={pid} STATUS={qstatus} EXIT={qexit} WALL_MS={wall:.3f}'); print('TRACE='+repr(dbg.read_text(encoding='ascii').splitlines() if dbg.exists() else [])); print(f'EVALUATOR_EXIT={eval_exit} STATIC_EXIT={static_exit} STAGE2_RAW_BYTES={len(b2)} RUNTIME_STATE_BYTES={runtime}'); print('INPUT_MANIFEST_SHA256='+msha); print('RECEIPT_SHA256='+sha(rp))
