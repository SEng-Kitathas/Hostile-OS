...
def main() -> int:
    ...
    base_path=BUILD/'d64_v2.img'; base=base_path.read_bytes(); bochs=find_bochs()
    stamp=datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    run=RUNS/f'{stamp}_h1_emulator_matrix_01'; run.mkdir(parents=True,exist_ok=False)
    source_head=git('rev-parse','HEAD'); body_tree=git('rev-parse','HEAD:os/research_only/d64_reference_v2')
    ...
    receipt={
        'format':'HOSTILE_OS_H1_EMULATOR_MATRIX_RUN_V1','status':'COMPLETED','passed':bool(qemu.get('passed')) and bochs_result['passed'],
