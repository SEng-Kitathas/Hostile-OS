...
H=Path(__file__).resolve().parent;B=H/'build_segment_encoding';B.mkdir(exist_ok=True)
...
run([clang,'-target','i386-unknown-none-elf','-ffreestanding','-c','stage1_segment_encoding.S','-o',B/'stage1.o'])
run([lld,'-m','elf_i386','-T','stage1_segment_encoding.ld',B/'stage1.o','-o',B/'stage1.elf'])
...
p=B/'h1_bios_usb_read_segment_encoding_1MiB.img';p.write_bytes(img)
