...
H=Path(__file__).resolve().parent;B=H/'build_no_recheck';B.mkdir(exist_ok=True)
...
run([clang,'-target','i386-unknown-none-elf','-ffreestanding','-c','stage1_no_recheck.S','-o',B/'stage1.o'])
run([lld,'-m','elf_i386','-T','stage1_no_recheck.ld',B/'stage1.o','-o',B/'stage1.elf'])
...
p=B/'h1_bios_usb_read_no_recheck_1MiB.img';p.write_bytes(img)
