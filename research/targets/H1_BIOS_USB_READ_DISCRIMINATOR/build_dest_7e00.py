...
H=Path(__file__).resolve().parent;B=H/'build_dest_7e00';B.mkdir(exist_ok=True)
...
run([clang,'-target','i386-unknown-none-elf','-ffreestanding','-c','stage1_dest_7e00.S','-o',B/'stage1.o'])
run([lld,'-m','elf_i386','-T','stage1_dest_7e00.ld',B/'stage1.o','-o',B/'stage1.elf'])
...
p=B/'h1_bios_usb_read_dest_7e00_1MiB.img'; p.write_bytes(img)
