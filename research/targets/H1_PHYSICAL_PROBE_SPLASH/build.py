...
def main():
 ...
 # Rebuild the parent probe from current committed source and bind to the already-qualified bytes.
 env=os.environ.copy()
 ...
 s1,_=compile_raw(clang,lld,obj,'wrapper_stage1.S','wrapper_stage1.ld','wrapper_stage1')
 loader,_=compile_raw(clang,lld,obj,'splash_loader.S','splash_loader.ld','splash_loader')
 b1=s1.read_bytes(); bl=loader.read_bytes(); pal=(HERE/'splash_palette_32xrgb6.bin').read_bytes(); pix=(HERE/'splash_pixels_320x200.bin').read_bytes()
 if len(b1)!=512 or b1[510:]!=b'\x55\xaa': raise SystemExit(f'bad wrapper stage1: bytes={len(b1)} sig={b1[510:].hex() if len(b1)>=512 else "short"}')
 if len(bl)>4096: raise SystemExit(f'loader too large: {len(bl)}')
 if len(pal)!=96 or len(pix)!=64000: raise SystemExit('splash asset size mismatch')
 def compose(probe:bytes,name:str):
  ...
  disk[9*512:9*512+len(pal)]=pal
  disk[10*512:10*512+len(pix)]=pix
  disk[135*512:135*512+len(probe)]=probe
  p=BUILD/name; p.write_bytes(disk); return p
 phys=compose(parent_phys.read_bytes(),'h1_probe_splash_physical.img')
 qemu=compose(parent_qemu.read_bytes(),'h1_probe_splash_qemu.img')
 sources=['H1_SPLASH_WRAPPER_PREREGISTRATION_2026-08-31.md','ASSET_PROVENANCE.md','wrapper_stage1.S','wrapper_stage1.ld','splash_loader.S','splash_loader.ld','build.py','splash_palette_32xrgb6.bin','splash_pixels_320x200.bin']
 manifest={'format':'HOSTILE_H1_SPLASH_WRAPPER_BUILD_V1','parent_probe':{'physical_stage2_sha256':sha(parent_phys),'qemu_stage2_sha256':sha(parent_qemu)},'wrapper_stage1':{'bytes':len(b1),'sha256':sha(s1)},'splash_loader':{'bytes':len(bl),'envelope':4096,'headroom':4096-len(bl),'sha256':sha(loader)},'palette':{'bytes':len(pal),'sha256':sha(HERE/'splash_palette_32xrgb6.bin')},'pixels':{'bytes':len(pix),'sha256':sha(HERE/'splash_pixels_320x200.bin')},'physical_image':{'bytes':phys.stat().st_size,'sha256':sha(phys)},'qemu_image':{'bytes':qemu.stat().st_size,'sha256':sha(qemu)},'layout':{'stage1_lba':0,'loader_lba_start':1,'loader_lba_count':8,'palette_lba':9,'pixels_lba_start':10,'pixels_lba_count':125,'probe_lba_start':135,'probe_lba_count':16},'sources':{x:sha(HERE/x) for x in sources},'tool_paths':{'clang':str(clang),'lld':str(lld),'objcopy':str(obj)}}
