...
def require_clean_source():
 cp=subprocess.run(['git','status','--porcelain','--','research/targets/H1_PHYSICAL_PROBE_DURABLE_LOG_TRACE_LBA257'],cwd=ROOT,text=True,stdout=subprocess.PIPE,check=True)
...
def main():
 ...
 files=['H1_DURABLE_LOG_TRACE_PREREGISTRATION_2026-08-31.md','wrapper_stage1.S','wrapper_stage1.ld','text_loader.S','text_loader.ld','stage2.S','stage2.ld','build.py','verify_static.py','extract_log.py','run_qemu_qualification.py']
 ...
 receipt={'format':'HOSTILE_H1_DURABLE_LOG_QEMU_DUAL_MODE_V1','source_head':git('rev-parse','HEAD'),'source_tree':git('rev-parse','HEAD:research/targets/H1_PHYSICAL_PROBE_DURABLE_LOG_TRACE_LBA257'),'input_hashes':{p.name:sha(p) for p in inp.iterdir() if p.is_file()},'results':results,'qualified':all(r['qualified'] for r in results)}
