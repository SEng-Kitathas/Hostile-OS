...
S_TRACE=['S1_8K_OK','TEST=H1_SMP_MIN02','IDS=0001','BAD=W01','GOOD=WW11','SMP_DONE']
...
def main():
 stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ'); run=RUNS/(stamp+'_h1_smp_min02_01'); inp=run/'inputs'; build=run/'build'; inp.mkdir(parents=True);build.mkdir()
 source_files=['stage1.S','stage1.ld','stage2.S','stage2.ld','launch.py','H1_SMP_MIN02_INTEGRATION_SPEC.md']; manifest=[]
 ...
 (run/'inputs_manifest.json').write_text(json.dumps({'format':'H1_SMP_MIN02_INPUTS_V1','git_head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'inputs':manifest},indent=2)+'\n')
