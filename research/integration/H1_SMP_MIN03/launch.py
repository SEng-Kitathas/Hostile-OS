...
S_TRACE=['S1_8K_OK','TEST=H1_SMP_MIN03','IDS=0001','OWNER=BSP','MAIL=WW11','SMP_DONE']
...
def main():
 stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ'); run=RUNS/(stamp+'_h1_smp_min03_01'); inp=run/'inputs'; build=run/'build'; inp.mkdir(parents=True);build.mkdir()
 source_files=['stage1.S','stage1.ld','stage2.S','stage2.ld','launch.py','H1_SMP_MIN03_INTEGRATION_SPEC.md']; manifest=[]
 ...
 (run/'inputs_manifest.json').write_text(json.dumps({'format':'H1_SMP_MIN03_INPUTS_V1','git_head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'inputs':manifest},indent=2)+'\n')
 ...
 scratch_used=syms['implementation_scratch_used_end']-syms['implementation_scratch']; src=(inp/'stage2.S').read_text(encoding='utf-8'); ap=src[src.index('smp_ap_worker:'):src.index('smp_ap_blob_start:')]; service=src[src.index('smp_service_mailbox:'):src.index('smp_start_ap1:')]; structural={'ap_no_relation_call':'call binding_attach_first' not in ap,'ap_no_legacy_input_scratch':all(x not in ap for x in ['input_activity_slot','input_activity_gen','input_activity_epoch','input_resource_slot','input_resource_gen','input_resource_epoch']),'payload_before_request':ap.rfind('movb $1,smp_mail_resource_epoch') < ap.index('movb $1,smp_mail_req'),'result_before_done':service.index('movb %al,smp_mail_result') < service.index('movb $1,smp_mail_done'),'no_candidate_a_gate':'smp_gate' not in src and 'xchgb' not in src}
 ...
 checks={'envelope_ok':envelope_ok,'state_bytes_unchanged':state_bytes==3467,'scratch_within_128':scratch_used<=128,**structural,'qemu_s_exact':qs['passed'],'qemu_c_exact':qc['passed'],'bochs_core_exact':br['checks']['core_exact'],'bochs_restart_exact':br['checks']['restart_exact_and_invariants'],'bochs_faults_exact':br['checks']['five_fault_cases_exact_and_readonly']}
 receipt={'format':'H1_SMP_MIN03_RUN_V1','status':'COMPLETED','passed':all(checks.values()),'checks':checks,'git_head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'stage2_raw_bytes':len(raw),'image_memory_bytes':image_bytes,'state_bytes':state_bytes,'implementation_scratch_used_bytes':scratch_used,'headroom_bytes':8192-image_bytes,'candidate_sha256':sha(build/'candidate.img'),'qemu_s':qs,'qemu_c':qc,'bochs':br};(run/'run_receipt.json').write_text(json.dumps(receipt,indent=2)+'\n'); print(json.dumps({'run':str(run.relative_to(ROOT)).replace('\\','/'),'passed':receipt['passed'],'checks':checks,'stage2_raw_bytes':len(raw),'image_memory_bytes':image_bytes,'implementation_scratch_used_bytes':scratch_used,'headroom_bytes':8192-image_bytes},indent=2));return 0 if receipt['passed'] else 1
