...
def qemu():
 v=os.environ.get('HOSTILE_QEMU_X64')
 if v and Path(v).is_file():return Path(v)
 q=shutil.which('qemu-system-x86_64') or shutil.which('qemu-system-x86_64.exe')
 if q:return Path(q)
 p=Path(r'C:\Program Files\qemu\qemu-system-x86_64.exe')
 if p.is_file():return p
...
def data_dir(q):
 ...
 return None
def main():
 stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ');run=RUNS/(stamp+'_c005_p20_01');inp=run/'inputs';build=run/'build';inp.mkdir(parents=True);build.mkdir()
 inputs=[HERE/'stage1.S',HERE/'stage1.ld',HERE/'stage2.S',HERE/'stage2.ld',HERE/'launch.py',HERE/'evaluate.py',HERE.parent/'P20_PREREGISTRATION.md',HERE.parent/'C005_CAMPAIGN_CHARTER_2026-08-31.md',ROOT/'research/targets/H1_HP_PAVILION_P2_1120_EMULATOR_MATRIX_V2.json'];rec=[]
 for p in inputs:
 ...
 (run/'inputs_manifest.json').write_text(json.dumps({'format':'C005_P20_INPUTS_V1','git_head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'inputs':rec},indent=2)+'\n',encoding='utf-8',newline='\n')
 ...
 q=qemu();dd=data_dir(q);dbg=run/'debugcon.txt';argv=[str(q)]
 if dd:argv+=['-L',str(dd)]
 ...
 receipt={'format':'C005_P20_RUN_V1','status':status,'exit_code':code,'argv':argv,'machine':'pc-q35-11.1','cpu_model':'phenom','vcpus':2,'memory_mib':4096,'accel':'tcg','target_role':'H1_QEMU_CONSTRAINT_PROXY_NOT_HARDWARE_IDENTITY','qemu_data_dir':str(dd) if dd else None,'stage1_bytes':len(s1),'stage2_bytes':len(s2),'stage1_sha256':sha(build/'stage1.bin'),'stage2_sha256':sha(build/'stage2.bin'),'trace':trace,'stderr':err.decode('utf-8',errors='replace') if isinstance(err,bytes) else str(err or '')};(run/'run_receipt.json').write_text(json.dumps(receipt,indent=2)+'\n',encoding='utf-8',newline='\n')
