...
def data_dir(q):
 ...
 return None
def main():
 stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ');run=RUNS/(stamp+'_c005_p02_01');inp=run/'inputs';build=run/'build';inp.mkdir(parents=True);build.mkdir()
 inputs=[HERE/'stage1.S',HERE/'stage1.ld',HERE/'stage2.S',HERE/'stage2.ld',HERE/'launch.py',HERE/'evaluate.py',HERE.parent/'P02_PREREGISTRATION.md',HERE.parent/'C005_CAMPAIGN_CHARTER_2026-08-31.md'];rec=[]
 for p in inputs:
 ...
 (run/'inputs_manifest.json').write_text(json.dumps({'format':'C005_P02_INPUTS_V1','git_head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'inputs':rec},indent=2)+'\n',encoding='utf-8',newline='\n')
