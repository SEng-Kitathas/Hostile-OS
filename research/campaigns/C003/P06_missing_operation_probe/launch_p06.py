...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: launch_p06.py RUN_ID", file=sys.stderr)
        return 64
    run_id = sys.argv[1]
    ...
    if len(boot) != 512 or boot[510:512] != b"\x55\xaa":
        print("boot image size/signature contract failed", file=sys.stderr)
        return 2
    ...
    qargv = [
        str(QEMU), "-accel", "tcg", "-display", "none", "-monitor", "none", "-serial", "none", "-no-reboot",
        "-drive", f"file={probe.as_posix()},format=raw,if=floppy",
        "-device", "isa-debug-exit,iobase=0xf4,iosize=0x04",
        "-debugcon", f"file:{debugcon.as_posix()}", "-global", "isa-debugcon.iobase=0xe9",
    ]
    started = now()
    ...
    eval_exit = None
    if debugcon.exists():
        eval_exit = run_capture([str(PYTHON), str(evaluator), str(debugcon), str(evaluation)], eval_stdout, eval_stderr)
    else:
    ...
    receipt = {
        "run_id": run_id,
        "run_class": "C003_P06_BOUNDED_MISSING_OPERATION_DISCRIMINATOR",
        "scientific_status": qstatus,
        "authority_ceiling": "bounded one-byte operation/result relation only",
        "cwd": str(repo),
