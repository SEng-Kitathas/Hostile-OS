...
def main():
    if len(sys.argv)!=2:
        print('usage: launch_p08.py RUN_ID',file=sys.stderr); return 64
    run_id=sys.argv[1]
    ...
    mechanism=src/'mechanism.S'; fixture=src/'fixture.S'; linker=src/'linker.ld'; evaluator=src/'evaluate_p08.py'; launcher=Path(__file__).resolve()
    ...
    receipt={
      'run_id':run_id,'run_class':'C003_P08_SELECTION_APPLICATION_SEPARATION_DISCRIMINATOR','scientific_status':qstatus,
      'authority_ceiling':'bounded two-eligible selection/application separation only','cwd':str(repo),
