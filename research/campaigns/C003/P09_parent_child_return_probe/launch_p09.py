...
def sha256(p:Path):
    ...
    return h.hexdigest()
def run_capture(argv,so,se,timeout=30):
    with so.open('wb') as o,se.open('wb') as e:
        return subprocess.run(argv,stdout=o,stderr=e,timeout=timeout,check=False).returncode

def main():
    if len(sys.argv)!=2:
        print('usage: launch_p09.py RUN_ID',file=sys.stderr); return 64
    run_id=sys.argv[1]
    src=Path(__file__).resolve().parent; repo=src.parents[3]
    ...
    mechanism=src/'mechanism.S'; fixture=src/'fixture.S'; linker=src/'linker.ld'; evaluator=src/'evaluate_p09.py'; launcher=Path(__file__).resolve()
    mo=run/'mechanism.o'; fo=run/'fixture.o'; elf=run/'probe.elf'; probe=run/'probe.bin'
    build=[
      ('01_clang_mechanism',[str(CLANG),'-target','i386-unknown-none-elf','-ffreestanding','-c',str(mechanism),'-o',str(mo)]),
      ('02_clang_fixture',[str(CLANG),'-target','i386-unknown-none-elf','-ffreestanding','-c',str(fixture),'-o',str(fo)]),
      ('03_link',[str(LLD),'-m','elf_i386','-T',str(linker),str(mo),str(fo),'-o',str(elf)]),
      ('04_objcopy',[str(OBJCOPY),'-O','binary',str(elf),str(probe)]),
    ]
    ...
    if len(boot)!=512 or boot[510:512]!=b'\x55\xaa':
        print(f'boot contract failed bytes={len(boot)} sig={boot[510:512].hex() if len(boot)>=512 else "short"}',file=sys.stderr); return 2
    ...
    with qso.open('wb') as o,qse.open('wb') as e:
        ...
        try: qexit=proc.wait(timeout=QEMU_TIMEOUT_SECONDS); qstatus='COMPLETED'
            ...
            proc.kill(); proc.wait(timeout=5); qexit=None; qstatus='UNKNOWN_TIMEOUT'
    ended=now()
    evaluation=run/'evaluation.json'; eso=run/'06_evaluator.stdout.txt'; ese=run/'06_evaluator.stderr.txt'; eval_exit=None
    if debug.exists(): eval_exit=run_capture([str(PYTHON),str(evaluator),str(debug),str(evaluation)],eso,ese)
    else: eso.write_bytes(b''); ese.write_text('debugcon missing; evaluator not run\n',encoding='utf-8')
    receipt={
      'run_id':run_id,'run_class':'C003_P09_LINEAGE_WAIT_WAKE_PARENT_RETURN_DISCRIMINATOR','scientific_status':qstatus,
      'authority_ceiling':'bounded one-parent/one-child lineage + generic wait/wake + separate application only','cwd':str(repo),
      'qemu':{'pid':pid,'argv':qargv,'started_utc':started,'ended_utc':ended,'status':qstatus,'exit_code':qexit,'timeout_seconds':QEMU_TIMEOUT_SECONDS},
      'tools':{'clang':{'path':str(CLANG),'sha256':sha256(CLANG)},'lld':{'path':str(LLD),'sha256':sha256(LLD)},'objcopy':{'path':str(OBJCOPY),'sha256':sha256(OBJCOPY)},'qemu':{'path':str(QEMU),'sha256':sha256(QEMU)},'python':{'path':str(PYTHON),'sha256':sha256(PYTHON)}},
      'source_sha256':{'mechanism':sha256(mechanism),'fixture':sha256(fixture),'linker':sha256(linker),'evaluator':sha256(evaluator),'launcher':sha256(launcher)},
