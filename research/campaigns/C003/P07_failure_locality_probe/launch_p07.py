...
def main():
    if len(sys.argv)!=2:
        print('usage: launch_p07.py RUN_ID',file=sys.stderr); return 64
    run_id=sys.argv[1]
    ...
    mechanism=src/'mechanism.S'; fixture=src/'fixture.S'; linker=src/'linker.ld'; evaluator=src/'evaluate_p07.py'; launcher=Path(__file__).resolve()
    ...
    receipt={
      'run_id':run_id,'run_class':'C003_P07_FAILURE_LOCALITY_LATER_PROGRESS_DISCRIMINATOR','scientific_status':qstatus,
      'authority_ceiling':'bounded fixed-capacity local-vs-global failure control only','cwd':str(repo),
