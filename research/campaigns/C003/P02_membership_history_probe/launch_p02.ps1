...
$evaluator = Join-Path $src 'evaluate_p02.py'
...
$receipt = [ordered]@{
    run_id = $RunId
    run_class = 'C003_P02_MEMBERSHIP_IDENTITY_HISTORY_DISCRIMINATOR'
    scientific_p02_completion = $true
    authority_ceiling = 'bounded fixed-capacity membership/history discriminator only; exact lost-C002 host reliance remains UNKNOWN'
    cwd = $repo
    started_utc = $started
    ended_utc = $ended
    tools = [ordered]@{
