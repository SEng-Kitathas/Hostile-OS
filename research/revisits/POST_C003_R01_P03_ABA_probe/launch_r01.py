...
def main():
 if len(sys.argv)!=2: print('usage: launch_r01.py RUN_ID',file=sys.stderr); return 64
 rid=sys.argv[1]; src=Path(__file__).resolve().parent; repo=src.parents[2]; run=repo/'research'/'revisits'/'runs'/rid
 if run.exists(): print(f'run directory already exists: {run}',file=sys.stderr); return 65
 run.mkdir(parents=True)
 mechanism=src/'mechanism.S'; fixture=src/'fixture.S'; linker=src/'linker.ld'; evaluator=src/'evaluate_r01.py'; static_checker=src/'static_check_r01.py'; launcher=Path(__file__).resolve()
 ...
 ended=now(); evaluation=run/'evaluation.json'; eso=run/'06_evaluator.stdout.txt'; ese=run/'06_evaluator.stderr.txt'; static=run/'07_static_closure.json'; sso=run/'07_static.stdout.txt'; sse=run/'07_static.stderr.txt'; eval_exit=None; static_exit=None
 ...
 static_exit=cap([str(PYTHON),str(static_checker),str(mechanism),str(static)],sso,sse)
 receipt={'run_id':rid,'run_class':'POST_C003_R01_P03_ABA_REVISIT','scientific_status':qstatus,'authority_ceiling':'bounded spanning-reader ABA/currentness revisit only','cwd':str(repo),'qemu':{'pid':pid,'argv':qargv,'started_utc':started,'ended_utc':ended,'status':qstatus,'exit_code':qexit,'timeout_seconds':QEMU_TIMEOUT_SECONDS},'tools':{'clang':{'path':str(CLANG),'sha256':sha256(CLANG)},'lld':{'path':str(LLD),'sha256':sha256(LLD)},'objcopy':{'path':str(OBJCOPY),'sha256':sha256(OBJCOPY)},'qemu':{'path':str(QEMU),'sha256':sha256(QEMU)},'python':{'path':str(PYTHON),'sha256':sha256(PYTHON)}},'source_sha256':{'mechanism':sha256(mechanism),'fixture':sha256(fixture),'linker':sha256(linker),'evaluator':sha256(evaluator),'static_checker':sha256(static_checker),'launcher':sha256(launcher)},'artifacts':{'probe_bin':{'path':str(probe),'bytes':len(boot),'sha256':sha256(probe),'boot_signature':'55aa'},'debugcon':{'path':str(debug),'sha256':sha256(debug) if debug.exists() else None},'evaluation':{'path':str(evaluation),'sha256':sha256(evaluation) if evaluation.exists() else None,'evaluator_exit':eval_exit},'static_closure':{'path':str(static),'sha256':sha256(static) if static.exists() else None,'checker_exit':static_exit},'qemu_stdout':{'path':str(qso),'sha256':sha256(qso)},'qemu_stderr':{'path':str(qse),'sha256':sha256(qse)},'evaluator_stdout':{'path':str(eso),'sha256':sha256(eso)},'evaluator_stderr':{'path':str(ese),'sha256':sha256(ese)},'static_stdout':{'path':str(sso),'sha256':sha256(sso)},'static_stderr':{'path':str(sse),'sha256':sha256(sse)}}}
 ...
 print(f'EVALUATOR_EXIT={eval_exit} STATIC_EXIT={static_exit}'); print('RECEIPT_SHA256='+sha256(rp))
 if qstatus=='UNKNOWN_TIMEOUT': return 3
 return 0 if qexit==33 and eval_exit==0 and static_exit==0 else 1
