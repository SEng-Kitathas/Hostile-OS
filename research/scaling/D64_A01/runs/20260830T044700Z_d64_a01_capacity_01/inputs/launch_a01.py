...
STAGE2_EXTENT=4096
QEMU_TIMEOUT=8
PREREG_COMMIT='eca4069643d81a5415d677c494c4b5ef8c305c3e'
...
def main():
 if len(sys.argv)!=2: print('usage: launch_a01.py RUN_ID',file=sys.stderr); return 64
 ...
 originals={
  'preregistration':src.parent/'D64_A01_PREREGISTRATION.md',
  'target_profile':repo/'research/plans/HOSTILE_OS_TARGET_WORKLOAD_PROFILE_D64_2026-08-30.md',
  'stage1_s':repo/'research/integration/I001/probe/stage1.S',
  'stage1_ld':repo/'research/integration/I001/probe/stage1.ld',
  'stage2_s':src/'stage2.S','stage2_ld':src/'stage2.ld','launcher':Path(__file__).resolve(),
  'evaluator':src/'evaluate_a01.py','static_checker':src/'static_check_a01.py'
 }
 snap_names={'preregistration':'preregistration.md','target_profile':'target_profile.md','stage1_s':'stage1.S','stage1_ld':'stage1.ld','stage2_s':'stage2.S','stage2_ld':'stage2.ld','launcher':'launch_a01.py','evaluator':'evaluate_a01.py','static_checker':'static_check_a01.py'}
 ...
 # Build only from snapshotted inputs.
 s1s=inp/'stage1.S'; s1ld=inp/'stage1.ld'; s2s=inp/'stage2.S'; s2ld=inp/'stage2.ld'; evaluator=inp/'evaluate_a01.py'; checker=inp/'static_check_a01.py'; launcher_snap=inp/'launch_a01.py'
 ...
 for name,argv in build:
  ...
  if rc!=0: failure_stage=name; break
 if failure_stage:
  (run/'failure.json').write_text(json.dumps({'stage':failure_stage,'input_manifest_sha256':manifest_sha},indent=2)+'\n'); print(failure_stage+' failed',file=sys.stderr); return 2
 # Protocol check: abort before QEMU if any controlling original changed after snapshot.
 ...
 if len(b1)!=512 or b1[-2:]!=b'\x55\xaa' or len(b2)>STAGE2_EXTENT: print('image contract failed',file=sys.stderr); return 2
 ...
 static=run/'static_closure.json'; sso=run/'09_static.stdout.txt'; sse=run/'09_static.stderr.txt'; static_exit=cap([str(PYTHON),str(checker),str(s2s),str(launcher_snap),str(evaluator),str(mp),str(static)],sso,sse)
 size_so=run/'10_size.stdout.txt'; size_se=run/'10_size.stderr.txt'; size_exit=cap([str(SIZE),str(s2elf)],size_so,size_se)
 nm_so=run/'11_nm.stdout.txt'; nm_se=run/'11_nm.stderr.txt'; nm_exit=cap([str(NM),'-n',str(s2elf)],nm_so,nm_se); runtime_bytes=None
 if nm_exit==0:
  sy={}
 ...
 receipt={'run_id':rid,'run_class':'D64_A01_ACTIVITY_CAPACITY_SCALING','scientific_status':qstatus,'authority_ceiling':'configured 64-slot activity scaling only','input_manifest_sha256':manifest_sha,'controlling_git_head':head,'controlling_preregistration_commit':PREREG_COMMIT,'qemu':{'pid':pid,'argv':argv,'started_utc':started,'ended_utc':ended,'wall_ms':wall,'status':qstatus,'exit_code':qexit,'timeout_seconds':QEMU_TIMEOUT},'source_sha256':snap_hashes,'tools':{k:{'path':str(v),'sha256':sha(v)} for k,v in {'clang':CLANG,'lld':LLD,'objcopy':OBJCOPY,'size':SIZE,'nm':NM,'qemu':QEMU,'python':PYTHON}.items()},'artifacts':{'stage1_bin':{'bytes':len(b1),'sha256':sha(s1bin)},'stage2_raw':{'bytes':len(b2),'sha256':sha(s2raw)},'stage2_padded':{'bytes':s2pad.stat().st_size,'sha256':sha(s2pad)},'disk':{'bytes':disk.stat().st_size,'sha256':sha(disk)},'debugcon':{'sha256':sha(dbg) if dbg.exists() else None},'evaluation':{'sha256':sha(evaluation) if evaluation.exists() else None,'exit':eval_exit},'static_closure':{'sha256':sha(static) if static.exists() else None,'exit':static_exit},'size':{'sha256':sha(size_so),'exit':size_exit},'nm':{'sha256':sha(nm_so),'exit':nm_exit}},'pareto':{'activity_capacity':64,'activity_field_species':11,'runtime_state_bytes':runtime_bytes,'generation_bits':8,'epoch_bits':8,'stage2_raw_bytes':len(b2)}}
