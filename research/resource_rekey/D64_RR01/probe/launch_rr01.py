...
STAGE2_EXTENT=8192
QEMU_TIMEOUT=12
PREREG_COMMIT='d293ecc46437a50fe642ea7dc944dc2213fe3b26'
...
def main():
 ...
 originals={
  'preregistration':src.parent/'D64_RR01_PREREGISTRATION.md',
  'target_profile':repo/'research/plans/HOSTILE_OS_TARGET_WORKLOAD_PROFILE_D64_2026-08-30.md',
  'resource_rekey_plan':repo/'research/plans/D64_RESOURCE_NAMESPACE_REKEY_PLAN_2026-08-30.md',
  ...
  'stage2_s':src/'stage2.S','stage2_ld':src/'stage2.ld',
  'launcher':Path(__file__).resolve(),'evaluator':src/'evaluate_rr01.py','static_checker':src/'static_check_rr01.py'}
 names={'preregistration':'preregistration.md','target_profile':'target_profile.md','resource_rekey_plan':'resource_rekey_plan.md','arb01_result':'arb01_result.md','arb01_adoption':'arb01_adoption.md','rb02_result':'rb02_result.md','loader_qualification_result':'loader_qualification_result.md','stage1_s':'stage1.S','stage1_ld':'stage1.ld','stage2_s':'stage2.S','stage2_ld':'stage2.ld','launcher':'launch_rr01.py','evaluator':'evaluate_rr01.py','static_checker':'static_check_rr01.py'}
 head=git_head(repo); items=[]; original_hash={}
 for k,p in originals.items():
  d=inp/names[k]; shutil.copyfile(p,d); original_hash[k]=sha(p); items.append({'key':k,'source_project_relative':p.relative_to(repo).as_posix(),'snapshot_path':d.relative_to(run).as_posix(),'bytes':d.stat().st_size,'sha256':sha(d)})
 ...
 s1s=inp/'stage1.S'; s1ld=inp/'stage1.ld'; s2s=inp/'stage2.S'; s2ld=inp/'stage2.ld'; evaluator=inp/'evaluate_rr01.py'; checker=inp/'static_check_rr01.py'; launcher_snap=inp/'launch_rr01.py'
 ...
 pre={'run_id':rid,'run_class':'D64_RR01_RESOURCE_NAMESPACE_REKEY','scientific_status':qstatus,'authority_ceiling':'bounded cooperative D64 resource namespace rekey only','input_manifest_sha256':msha,'controlling_git_head':head,'controlling_preregistration_commit':PREREG_COMMIT,'qemu':{'pid':pid,'argv':qargv,'started_utc':started,'ended_utc':ended,'wall_ms':wall,'status':qstatus,'exit_code':qexit,'timeout_seconds':QEMU_TIMEOUT},'source_sha256':source,'tools':{k:{'path':str(v),'sha256':sha(v)} for k,v in {'clang':CLANG,'lld':LLD,'objcopy':OBJCOPY,'size':SIZE,'nm':NM,'qemu':QEMU,'python':PYTHON}.items()},'artifacts':{'stage1_bin':{'bytes':len(b1),'sha256':sha(s1bin),'boot_signature':'55aa'},'stage2_raw':{'bytes':len(b2),'sha256':sha(s2raw)},'stage2_padded':{'bytes':len(s2pad.read_bytes()),'sha256':sha(s2pad)},'disk':{'bytes':len(img),'sha256':sha(disk)},'debugcon':{'sha256':sha(dbg) if dbg.exists() else None},'evaluation':{'sha256':sha(evaluation) if evaluation.exists() else None,'exit':eval_exit},'size':{'sha256':sha(size_so),'exit':size_exit},'nm':{'sha256':sha(nm_so),'exit':nm_exit}},'pareto':{'stage2_raw_bytes':len(b2),'stage2_extent_bytes':STAGE2_EXTENT,'runtime_state_bytes':runtime,'activity_capacity':64,'bindings_per_activity':20,'binding_cell_count':1280,'resource_capacity':64,'resource_rekey_binding_scan':1280,'resource_scan_reset':64,'observed_activity_epoch_after_rekey':obs.get('A_EPOCH_AFTER'),'observed_resource_epoch_after_rekey':obs.get('NEW_REPOCH'),'observed_binding_generation_after_rekey':obs.get('BGEN_AFTER_REKEY')}}
