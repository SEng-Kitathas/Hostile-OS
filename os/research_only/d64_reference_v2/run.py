...
def qemu_env(qemu):
 env=os.environ.copy();explicit=os.environ.get('HOSTILE_QEMU_MODULE_DIR') or os.environ.get('QEMU_MODULE_DIR')
 if explicit:env['QEMU_MODULE_DIR']=explicit;return env,explicit
 for c in (qemu.parent/'modules',qemu.parent.parent/'modules'):
  if c.is_dir():env['QEMU_MODULE_DIR']=str(c);return env,str(c)
 return env,None
def crc16(data):
 c=0xffff
 for b in data:
...
def run_core(base,q,env):
 d=RUNS/'core';d.mkdir(parents=True,exist_ok=True);disk=d/'core.img';make_image(base,disk,'C');return boot(q,env,disk,d/'debugcon.txt',True)
def run_restart(base,q,env):
 d=RUNS/'restart';d.mkdir(parents=True,exist_ok=True);disk=d/'restart.img';make_image(base,disk,'R',bytes(SECTOR),bytes(SECTOR));initial=sha(disk);b1=boot(q,env,disk,d/'boot1.txt',False);after1=sha(disk);a1=sector(disk,17);b1sec=sector(disk,18);expected_a=record(1,0x71,1,1);before2=sha(disk);b2=boot(q,env,disk,d/'boot2.txt',True);after2=sha(disk);return {'initial_disk_sha256':initial,'boot1':b1,'a_after_boot1_sha256':sha_bytes(a1),'a_after_boot1_exact_expected':a1==expected_a,'b_after_boot1_zero':b1sec==bytes(SECTOR),'disk_sha256_after_boot1':after1,'no_host_write_between_boots':before2==after1,'boot2':b2,'disk_unchanged_during_recovery_boot':before2==after2,'disk_sha256_after_boot2':after2}
def run_faults(base,q,env):
 ...
 for name,(a,b) in cases.items():
  d=RUNS/'faulted_media'/name;d.mkdir(parents=True,exist_ok=True);disk=d/(name+'.img');make_image(base,disk,'F',a,b);before=sha(disk);br=boot(q,env,disk,d/'debugcon.txt',True);out[name]={'boot':br,'disk_unchanged':before==sha(disk),'a_sha256':sha_bytes(a),'b_sha256':sha_bytes(b)}
 return out
def main()->int:
 ap=argparse.ArgumentParser();ap.add_argument('--mode',choices=['all','core','restart','faulted-media'],default='all');args=ap.parse_args();base_path=BUILD/'d64_v2.img';assert base_path.is_file(),'run build.py first';base=base_path.read_bytes();RUNS.mkdir(exist_ok=True);q=find_qemu();env,module=qemu_env(q);receipt={'format':'HOSTILE_OS_D64_V2_RUN_V2','body_status':'CURRENT_RESEARCH_REFERENCE','mode':args.mode,'qemu_module_dir':module,'base_disk_sha256':sha(base_path)}
 if args.mode in ('all','core'):receipt['core']=run_core(base,q,env)
 if args.mode in ('all','restart'):receipt['restart']=run_restart(base,q,env)
 if args.mode in ('all','faulted-media'):receipt['faulted_media']=run_faults(base,q,env)
